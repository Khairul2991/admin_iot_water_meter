...
const EditUser = () => {
      ...
      try {
        ...
        if (location.state && location.state.data) {
          ...
          setFormData({
            ...
            province: location.state.data.province || "",
  ...
  // Tambahkan fungsi capitalizeWords di bagian atas komponen
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.patch(
        `http://localhost:5000/api/edit-user/${formData.uid}`,
        {
          name: formData.name,
        ...
        <div className="form-1 bg-white shadow overflow-hidden sm:rounded-lg p-4 md:p-8">
          ...
          <form onSubmit={handleSubmitWithConfirmation}>
            <div className="form-2 bg-white shadow-xl overflow-hidden sm:rounded-lg p-6 my-2 md:my-4">
              <h1 className="text-md md:text-xl font-bold mb-6 text-start">
                User Information
              </h1>
